...
from ..views_3D.ellipsoid_view_3D import EllipsoidView3D

class EllipsoidItem(GraphicsItem):
    '''
    This is a derivative of the GrahicsItem class
    that will handle the drawing of a Ellipsoid
    on a 3D OpenGl canvas
    '''
    ...
    def drawGL(self, target_view = None):
        ...
        if self['Visible']:
            self.draw_items = [EllipsoidView3D()]
            self.default_target.addItem(self.draw_items[0])
            self.setOpenGLMode()
            self.setVisual()
