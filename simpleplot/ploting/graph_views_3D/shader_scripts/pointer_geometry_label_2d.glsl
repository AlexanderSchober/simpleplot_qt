...
void main()
{
    ...
    float x_pos_init = position.x;
    if (x_pos <= 0){
        x_pos += height_scene;
        x_pos_init += height_scene;
    } else {
        x_pos -= width_pixel*viewport_pixel_size.x + height_scene;
        x_pos_init -= label_box_width*viewport_pixel_size.x + height_scene;
    };

    float y_pos = position.y;
    if (y_pos <= 0.000001){
        if (int(position[2]) == 0) {
            y_pos += 2*height_scene;
        } else {
            y_pos += height_scene;
        }} else {        
        if (int(position[2]) == 0) {
    ...
    vec2 init_pos = vec2(x_pos_init, y_pos);
    ...
    if (draw_label_box == 1) {
        type = 1;        
        gl_Position = vec4(vec2(init_pos.x, init_pos.y+height_scene/2),z_near+0.0001,1);
        EmitVertex();
        gl_Position = vec4(vec2(init_pos.x + (label_box_width)*viewport_pixel_size.x, init_pos.y+height_scene/2),z_near+0.0001,1);
        EmitVertex();
        gl_Position = vec4(vec2(init_pos.x, init_pos.y-height_scene/2),z_near+0.0001,1);
        EmitVertex();
        gl_Position = vec4(vec2(init_pos.x + (label_box_width)*viewport_pixel_size.x, init_pos.y-height_scene/2),z_near+0.0001,1);
