    ...
    def drawPath(self):
        ...
        if height is not None or top is not None or bottom is not None:
            ...
            if beam is not None and beam > 0:
                x1 = x - beam/2. if not self.opts['logMode'][1] else x - 10**(beam/2.)
                x2 = x + beam/2. if not self.opts['logMode'][1] else x + 10**(beam/2.)
        ...
        if width is not None or right is not None or left is not None:
            ...
            if beam is not None and beam > 0:
                y1 = y - beam/2. if not self.opts['logMode'][1] else y - 10**(beam/2.)
                y2 = y + beam/2. if not self.opts['logMode'][1] else y + 10**(beam/2.)
