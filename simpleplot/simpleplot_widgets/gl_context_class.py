...
class ContextClass(QtCore.QObject):
    ...
    def setMVP(self, item = None)->None:
        ...
        for item in items:
            if hasattr(item, 'setMVP'):
                item.setMVP(
                    self._camera.mat_projection, 
                    self._camera.mat_lookat,
                    np.array(self._camera['Center position']),
                    self._camera.vec_view_pos)
    ...
    def resizeEvent(self, x:float,y:float,width:float,height:float)->None:
        ...
        if not self.context() is None:
            ...
            if not self._camera is None and not self.context().viewport[3] == 0:
    ...
    def getPickingRay(self,x:int,y:int):
        ...
        model_mat   = self._camera.mat_lookat.inverse
