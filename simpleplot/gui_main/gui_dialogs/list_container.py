...
class ListContainer:
    ...
    def __init__(self, list_view):
        '''
        This method will set the list view and wrap
    ...
    def build_container(self, **kwargs):
        '''
        Creation of the coantainer for a specific list
        '''
        ...
        if not self.dictionary['clicked'] == None:
            self.dictionary['view'].clicked.connect(
                self.dictionary['clicked'])

        if not self.dictionary['itemchanged'] == None:
            self.dictionary['model'].itemChanged.connect(
                self.dictionary['itemchanged'])
    ...
    def add_item_to_list(self, item):
        ...
        if self.dictionary['check']:
            ...
            if self.dictionary['true']:
                self.dictionary['items'][-1][0].setCheckState(
                    QtCore.Qt.Checked)
