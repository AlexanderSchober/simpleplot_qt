...
class TriangleView3D(GraphicsView3D):
    '''
    ...
    def initializeGL(self)->None:
        ...
        self._createVBO("tiles", np.zeros((3,3)), np.zeros((3,3)), np.zeros((3,4)))
    ...
    def _generateBases(self):
        ...
        easier afterwards
        '''
        self._base = np.zeros((3,3),dtype=np.float)
        self._base[0] = [-1/2, -1/3, 0]
        self._base[1] = [+1/2, -1/3, 0]
        self._base[2] = [ 0, +2/3, 0]
        self._line_base = np.array([0,1,1,2,2,0])
    ...
    def _processTiles(self):
        ...
        '''
        self._vertices  = self._base[:] * np.array(self._parameters['diameters']+[1])
        self._normals   = np.array([[0,0,1],[0,0,1],[0,0,1]])
